"""Spectrum-facing routes for messaging-driven Lazarus actions."""
...
@router.get("/health")
def spectrum_health() -> dict[str, str]:
    return {"status": "ok", "message": "Lazarus Spectrum bridge is ready."}


@router.post("/webhook", response_model=schemas.SpectrumWebhookResponse)
def spectrum_webhook(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Unsupported Spectrum action. Use action='review' or 'blueprint', or text like 'review RX-782'.",
        )
